...
const SectionOneBotSA = (id) => {
  ...
  const handleSubmitEditBot = (e) => {
    e.preventDefault()
    setLoading(true);
    axios.post(`${BACKEND}api/admin/updatebot/${id.id}`, { botname, prompt, initialMsg, suggestedQuery }, {
      'Content-type': 'application/json',
      'Accept': 'application/json',
      'Access-Control-Allow-Origin': '*'
              ...
              filteredDataIssue.length === 0 ?
                ...
                itemsToDisplayDataIssue.map(x => (
                  <div className='row col-12 mt-3' style={{ borderBottom: '1px solid white', display: 'flex', flexWrap: 'wrap' }}>
                    ...
                    <div className='col-lg-2 col-sm-6'>
                      {/* <Link to={`/superadminBot/${x.bot_id}`} style={{ textDecoration: 'none' }}><Nav.Link href="#link" className='col-12 d-flex justify-content-start text-start text-decoration-underline' style={{ color: '#FFFFFF' }}>Details</Nav.Link></Link> */}
                      <a href="#" onClick={() => handleDetailsClickIssue(x)} className='col-12 d-flex justify-content-start text-start text-decoration-underline' style={{ color: '#FFFFFF' }}>Details</a>
...
<div className='fs-4 col-12 pb-5 d-flex justify-content-center text-center mb-5 mt-0' style={{ color: '#FFFFFF', backgroundColor:'#242439' }}>
      ...
      <div >
          ...
          <input className='fs-4 d-flex justify-content-center container mt-1 text-center  mb-3' style={{ width: '95%'}} readOnly placeholder='PDF Used' value={data.pdf} />
          ...
          <textarea className='fs-4 d-flex justify-content-center container mt-1 text-center  ' style={{ width: '95%', marginBottom:'100px' }} readOnly placeholder='Urls excluded'  value={Array.isArray(data.urlsEx) ? data.urlsEx.join('\n') : ''} />
